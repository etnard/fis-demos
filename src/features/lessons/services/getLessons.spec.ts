...
describe("getLessons", () => {
  const mockRows: LessonsRow[] = [
    factory.build("lessons", { id: crypto.randomUUID() }),
    factory.build("lessons", { id: crypto.randomUUID() }),
  ];
  ...
  it("fetches lessons and returns lessons instances", async () => {
    ...
    result.forEach((item, index) => {
      expect(item).toBeInstanceOf(Lessons);
  ...
  it("throws an error when the fetch response is not ok", async () => {
    ...
    await expect(getLessons()).rejects.toThrow("Failed to fetch lessons");
  });
});
